...
const RiskEngine = (function () {
    ...
    function calculateIntentScore(intentAnalysis) {
        ...
        // Manipulation level
        const manipAnalysis = intentAnalysis.manipulationAnalysis || { manipulationLevel: 'low' };
        const manipLevel = manipAnalysis.manipulationLevel;
        let manipScore = 0;
        ...
        // Urgency indicators
        if (intentAnalysis.urgencyAnalysis && intentAnalysis.urgencyAnalysis.totalIndicators > 0) {
        ...
        if (intentAnalysis.actionAnalysis && intentAnalysis.actionAnalysis.detected && intentAnalysis.actionAnalysis.detected.CRITICAL.length > 0) {
    ...
    function calculateRiskScore(url, urlAnalysis, intentAnalysis, infraAnalysis = {}) {
        ...
        // SAFEGUARD: If this is the actual legitimate domain of a known brand, override the score to safe
        // to prevent false positives on their legitimate login or action pages
        if (urlAnalysis && urlAnalysis.features && urlAnalysis.features.brand && urlAnalysis.features.brand.isLegitimateDomain) {
            totalScore = 0;
        }
